...
model = nn.Sequential(nn.Linear(150528, 37632),
                      nn.ReLU(),
                      nn.Dropout(p=0.2),
                      nn.Linear(37632, 9408),
                      nn.ReLU(),
                      nn.Dropout(p=0.2),
                      nn.Linear(9408, 1024),
                      nn.ReLU(),
...
print("\n Training NN from training data (5 hidden layers, 5 epochs, optim.Adam, lr 0.005)...\n")
...
torch.save(model.state_dict(), 'checkpoint_CatDog.pth')

print("\n...and we're done with calculations. Now to plot the losses and training times.\n")

plt.plot(train_losses, label='Training loss')
plt.plot(test_losses, label='Validation loss')
plt.plot(epoch_trainingtime, label='Epoch training time')
